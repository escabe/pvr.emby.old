...
struct PVREmbyChannel
{
  ...
  std::vector<PVREmbyEpgEntry> epg;
};

struct PVREmbyRecording
{
  bool        bRadio;
  int         iDuration;
  int         iGenreType;
...
public:
  PVREmbyData(void);
  virtual ~PVREmbyData(void);

  virtual int GetChannelsAmount(void);
  virtual PVR_ERROR GetChannels(ADDON_HANDLE handle, bool bRadio);
  virtual bool GetChannel(const PVR_CHANNEL &channel, PVREmbyChannel &myChannel);
...
private:
  ...
  std::vector<PVREmbyRecording>    m_recordings;
  std::vector<PVREmbyRecording>    m_recordingsDeleted;
  std::vector<PVREmbyTimer>        m_timers;
  time_t                           m_iEpgStart;
