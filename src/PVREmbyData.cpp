...
std::string PVREmbyData::GetSettingsFile() const
{
  ...
  if (pElement)
  {
    ...
    while ((pEpgNode = pElement->IterateChildren(pEpgNode)) != NULL)
    {
      ...
      if (!XMLUtils::GetInt(pEpgNode, "channelid", iTmp))
        continue;
      PVREmbyChannel &channel = m_channels.at(iTmp - 1);
...
int PVREmbyData::GetChannelsAmount(void)
{
  return m_channels.size();
}

PVR_ERROR PVREmbyData::GetChannels(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVREmbyChannel &channel = m_channels.at(iChannelPtr);
...
bool PVREmbyData::GetChannel(const PVR_CHANNEL &channel, PVREmbyChannel &myChannel)
{
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVREmbyChannel &thisChannel = m_channels.at(iChannelPtr);
...
int PVREmbyData::GetChannelGroupsAmount(void)
{
  return m_groups.size();
}

PVR_ERROR PVREmbyData::GetChannelGroups(ADDON_HANDLE handle, bool bRadio)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVREmbyChannelGroup &group = m_groups.at(iGroupPtr);
...
PVR_ERROR PVREmbyData::GetChannelGroupMembers(ADDON_HANDLE handle, const PVR_CHANNEL_GROUP &group)
{
  for (unsigned int iGroupPtr = 0; iGroupPtr < m_groups.size(); iGroupPtr++)
  {
    PVREmbyChannelGroup &myGroup = m_groups.at(iGroupPtr);
    if (!strcmp(myGroup.strGroupName.c_str(),group.strGroupName))
    {
      for (unsigned int iChannelPtr = 0; iChannelPtr < myGroup.members.size(); iChannelPtr++)
      {
        ...
        if (iId < 0 || iId > (int)m_channels.size() - 1)
          continue;
        PVREmbyChannel &channel = m_channels.at(iId);
...
PVR_ERROR PVREmbyData::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL &channel, time_t iStart, time_t iEnd)
{
  if (m_iEpgStart == -1)
    m_iEpgStart = iStart;
  ...
  for (unsigned int iChannelPtr = 0; iChannelPtr < m_channels.size(); iChannelPtr++)
  {
    PVREmbyChannel &myChannel = m_channels.at(iChannelPtr);
    ...
    while (iLastEndTime < iEnd && myChannel.epg.size() > 0)
    {
      ...
      {
        PVREmbyEpgEntry &myTag = myChannel.epg.at(iEntryPtr);
...
int PVREmbyData::GetRecordingsAmount(bool bDeleted)
{
  return bDeleted ? m_recordingsDeleted.size() : m_recordings.size();
}

PVR_ERROR PVREmbyData::GetRecordings(ADDON_HANDLE handle, bool bDeleted)
{
  std::vector<PVREmbyRecording> *recordings = bDeleted ? &m_recordingsDeleted : &m_recordings;

  for (std::vector<PVREmbyRecording>::iterator it = recordings->begin() ; it != recordings->end() ; it++)
  {
    PVREmbyRecording &recording = *it;
...
int PVREmbyData::GetTimersAmount(void)
{
  return m_timers.size();
}

PVR_ERROR PVREmbyData::GetTimers(ADDON_HANDLE handle)
{
  unsigned int i = PVR_TIMER_NO_CLIENT_INDEX + 1;
  for (std::vector<PVREmbyTimer>::iterator it = m_timers.begin() ; it != m_timers.end() ; it++)
  {
    PVREmbyTimer &timer = *it;
